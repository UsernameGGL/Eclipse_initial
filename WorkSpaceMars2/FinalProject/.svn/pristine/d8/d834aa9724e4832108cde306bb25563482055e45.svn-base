 ...
 */
	 ...
	 */
		if(activeSum>0){
			...
			document.getElementById(label.id+"_1").innerHTML="<input name=\"_"+label.id+"_1\" value=\""+ oldJpid+"\" />";
			document.getElementById(label.id+"_3").innerHTML="<input name=\"_"+label.id+"_2\" value=\""+ oldJsal+"\" />";
			myLabel=label;
			activeSum=1;
		}
	}
 ...
 */
	...
	function keep(label){
		if(activeSum>0&&myLabel.id==label.name){
			...
			activeSum=0;
			myLabel="";
			location="/jobInfo/jobInfo_update.action?jobInfo.jid="+label.name+"&jobInfo.jpid="+newJpid+"&jobInfo.jsal="+newJsal;
		}
	}
	function del(label){
		if(activeSum>0){
