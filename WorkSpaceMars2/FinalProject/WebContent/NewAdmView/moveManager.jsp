		...
		<tbody>
			...
			<c:if test="${ not  empty  requestScope.salList}">
				...
				<c:forEach items="${  requestScope.salList }" var="j">
					...
					<td><b>${j.jnm }</b></td>
					<td><b>${j.jage }</b></td>
					<td id="${j.jid }_1"><b>${j.pnm }</b></td>
					<td id="${j.jid }_2"><b>${j.dnm }</b></td>
					<td id="${j.jid }_3"><b>${j.jsal }</b></td>
					<td><b><a id="${j.jid }"  onclick="writeEnable(this)" 
					style="cursor:pointer;color:white">编辑</a>
					<a name="${j.jid }"  onclick="cancel(this)"
					style="cursor:pointer;color:white">取消编辑</a>
					<a name="${j.jid }" style="cursor:pointer;color:white" onclick="keep(this)">保存</a>
					<a name="${j.jid}" onclick="del(this)" style="cursor:pointer;color:white">辞退</a></b>
					</td>
					</tr>
