		...
		<tbody>
			...
			<c:if test="${ not  empty  requestScope.listDept}">
				<c:forEach items="${  requestScope.listDept }" var="p">
					<tr>
						<td><b>${p.did }</b></td>
						<td  id="${p.did }_1"><b>${p.dnm }</b></td>
						<td id="${p.did }_2"><b>${p.dnum }</b></td>
						
						<td> <a id="${p.did }"  onclick="writeEnable(this)" 
						style="cursor:pointer;color:white"><b>编辑</b></a>
						<a name="${p.did }"  onclick="cancel(this)"
						style="cursor:pointer;color:white"><b>取消编辑</b></a>
						...
						style="cursor:pointer;color:white"><b>删除</b></a>
						<a style="color:white" href="dept_listperson.action?did=${p.did }"><b>下属员工</b></a></td>
