		...
		<tbody>
			...
			<c:if test="${ not  empty  requestScope.listPosition}">
				<c:forEach items="${  requestScope.listPosition }" var="p">
					<tr>
						<td><b>${p.pid }</b></td>
						<td id="${p.pid }_1"><b>${p.pnm }</b></td>
						<td id="${p.pid }_2"><b>${p.pnum }</b></td>
						<td> <b><a id="${p.pid }"  onclick="writeEnable(this)" 
						...
						style="cursor:pointer;color:white">删除</a>
							<a style="color:white" href="position_listperson.action?pid=${p.pid }">在职员工</a>
						</b></td>
					</tr>
				</c:forEach>
			</c:if>
